...
export default function OrderDetails({ route }: Props) {
  ...
  const handleOnConfirmation = () => {
    confirmDelivery(order.id)
      .then(() => {
        Alert.alert(`Entrega do pedido ${order.id} confirmado com sucesso!`);
        navigation.navigate("Orders");
      })
      .catch(() => {
        Alert.alert(`Houve um erro a confirma a entrega do pedido ${order.id}`);
      })
  }
  ...
  return (
    <>
      ...
      <View style={styles.container}>
        ...
        <RectButton style={styles.button} onPress={handleOnConfirmation}>
